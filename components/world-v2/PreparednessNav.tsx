...
/**
 * Navegação local da Preparação (PREP-T07 fase 1 / D-164).
 *
 * ── Por que chips com ROTA, e não abas ─────────────────────────────────────
 *
 * Cada subtópico é um endereço de verdade. O Pilot salva itens no checklist e
 * precisa poder apontar para eles; o manifesto do PWA tem atalhos; convites e
 * push chegam em rotas profundas. Estado de navegação que só existe em memória
 ...
 * leitor de tela uma troca de aba que na verdade é uma navegação — mentir para
 * a tecnologia assistiva é pior do que não usar o padrão.
 *
 * ── Por que também há portas na Visão ──────────────────────────────────────
 *
 * Os chips ficam no topo, fora do arco do polegar. Na primeira visita e no uso
 * de uma mão, o caminho principal são os cartões dentro da rolagem da Visão.
 * Os chips são o caminho de repetição, para quem já sabe onde vai. Duas rotas
 * para o mesmo lugar, cada uma boa num momento diferente — é troca deliberada,
 * não acidente.
 */
...
export default function PreparednessNav() {
  ...
  return (
    <nav ref={faixa} aria-label={language === 'pt' ? 'Seções da Preparação' : 'Preparedness sections'} style={S.faixa}>
