...
async function notifySavedEdu(
  admin: NonNullable<ReturnType<typeof createAdminClient>>,
  actorId: string,
  content: { id: string; title: string; version?: number | null; status?: string | null },
) {
  await createCommsNotifications({
    admin,
    actorId,
    ...
    kind: 'edu_content_saved',
    title: `EDU salvo: ${content.title}`,
    body: `Status: ${content.status ?? 'draft'} · v${content.version ?? 1}`,
    href: `/admin/edu?contentId=${encodeURIComponent(content.id)}`,
    sourceKey: `edu-save:${content.id}:v${content.version ?? 1}:${actorId}`,
    excludeActor: false,
    metadata: {
...
export async function POST(req: NextRequest) {
  ...
  if (body.id) {
    ...
    const { data, error } = await admin
      ...
      .single()
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
    await notifySavedEdu(admin, user.id, data as { id: string; title: string; version?: number | null; status?: string | null })
