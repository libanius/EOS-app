...
Implementation note as of 2026-07-21 (D-054→D-058): HWD-06 starts with the responsive HUD pass because usability is currently blocking mobile and smaller desktop testing. Scope delivered so far: mobile bottom sheet with `peek`/`half`/`full` states, desktop HUD collapse, map-interaction-driven panel collapse, Status Rail redesign into the approved household readiness card, removal of the central "Your Area/Sua área" visual title, and an initial validation report. HWD-06 remains open for the rest of the production exit criteria. See `docs/17-hwd-06-validation.md`.
