...
| **Last Completed Task** | **D-141 / WV2-T13 — Vento animado corrigido com grid por viewport e cobertura real (2026-08-09)** |
...
| **Build** | ✅ WV2-T13 validado em 2026-08-09 com `npm run type-check`, `npm test -- --runInBand` (202/202), `npm run build`, `npm run test:weather` (vento por viewport: 81 leituras, 252 partículas, raio de cobertura), `npm run test:nav`, `node scripts/pilot-orb-test.mjs` e `git diff --check`. |
