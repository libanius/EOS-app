...
| **⚠️ Pendência — notificações de clima** | O agendador da Vercel ficou **diário** (`0 11 * * *`) porque o plano Hobby não aceita mais que isso, e um deploy com `*/15` **bloqueia toda publicação**. A cadência real de 15 min vem do GitHub Actions (`.github/workflows/weather-notifications.yml`), de graça. **Falta o dono adicionar o secret `CRON_SECRET` no GitHub** (Settings → Secrets and variables → Actions), com o mesmo valor já configurado na Vercel — sem ele o workflow falha alto, de propósito. Decidir depois: Vercel Pro traz o agendador de volta para casa e elimina a dependência do GitHub. |
