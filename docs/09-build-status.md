...
| **Build** | ✅ WV2-T27 validado em 2026-08-15 com `npm run type-check`, `git diff --check`, `npm run build`, `npm run test:nav` e Playwright no dashboard: mobile `peek → hidden → peek` por toque; mouse `hidden → peek` por hover. |
