...
| **Last Completed Task** | **D-121 / PREP-T02 — Editar e excluir itens do checklist em Preparação; controles visíveis por linha (2026-08-04)** |
...
| **Next Task** | Roadmap sem tarefa `PENDING/IN PROGRESS` após PREP-T02; próxima frente precisa ser promovida explicitamente no Spine antes de código. |
| **Build** | ✅ PREP-T02 validado em 2026-08-04 com `npm run type-check`, `npm test -- --runInBand` (91/91), `npm run build` e `git diff --check`; UX corrigida para mostrar `Editar`/`Excluir` como botões textuais. |
