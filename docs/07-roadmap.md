...
| NAV-T09 | BottomNav vira pílula expansível | ✅ COMPLETE | 2026-08-15 — D-189. Pedido do dono: copiar o comportamento/estilo do componente React de referência, mantendo os ícones atuais e o MUNDO verde. A barra real ganhou spring de entrada, toque com `whileTap` e rótulo ativo expansível; ativo segue derivado da rota, não de `useState`; badges, i18n, deep-links e os cinco destinos de D-180 ficaram intactos. Sem `lucide-react`: `framer-motion` já existia e os SVGs atuais foram preservados. Validado com `type-check`, build, `test:nav` e captura mobile 320px |
