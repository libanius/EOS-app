...
| NAV-T09 | BottomNav vira pílula expansível | ✅ COMPLETE | 2026-08-15 — D-189. Pedido do dono: copiar o comportamento/estilo do componente React de referência, mantendo os ícones atuais e o MUNDO verde. A barra real ganhou spring de entrada, toque com `whileTap` e rótulo ativo expansível; ativo segue derivado da rota, não de `useState`; badges, i18n, deep-links e os cinco destinos de D-180 ficaram intactos. **Follow-up D-191:** a cápsula central ficou pequena demais; a barra voltou a ocupar a largura inteira do rodapé, sem bordas externas arredondadas, mantendo o comportamento expansível. Validado com `type-check`, build, `test:nav` e captura mobile 320px |
...
| COMMS-T11 | A conversa vira uma coisa (fase 1 de 3) | ✅ COMPLETE (migração **pendente de aplicação**) | 2026-08-15 — D-188. Pedido do dono: chat individual com gente do círculo, além do grupo. Hoje o thread é **implícito** (`circle_messages.circle_id`) e a tela usa `?view=` em memória. Critérios: (1) `conversations` + `conversation_members` + `circle_messages.conversation_id`, **aditivo e idempotente**; (2) toda conversa nasce dentro de um círculo — a regra de permissão continua sendo UMA (a de D-073); (3) `direct_key` = os dois `user_id` **ordenados**, único por círculo, para que abrir pelos dois lados encontre o mesmo thread; (4) **"excluir" esconde para mim** (`hidden_at`), nunca destrói para todos — histórico compartilhado é registro de quem avisou o quê e quando; (5) mensagem nova reabre a escondida; (6) backfill cria uma conversa por círculo e **falha alto** se sobrar mensagem órfã; (7) nenhuma tela muda ainda. **Entregue** 2026-08-15: `20260815000000_conversations.sql`, `lib/conversations.ts`, 29 testes. 429 testes unitários. **Pendência: aplicar a migração no SQL Editor** |
| COMMS-T12 | Lista de conversas e thread viram ROTAS (fase 2 de 3) | PENDING | `/comms` = lista, `/comms/[id]` = thread, `/comms/radio` e `/comms/linha-do-tempo` saem de `?view=`. COMMS ganha a faixa de domínio — **é o último domínio sem uma**. Inspiração visual: lista com avatar, nome, prévia e hora (referência do dono). Depende da migração aplicada |
| COMMS-T13 | Conversa individual e esconder (fase 3 de 3) | PENDING | Abrir 1:1 com alguém do círculo (find-or-create pela `direct_key`), estado de leitura por conversa e "excluir" = esconder. Depende de T12 |
