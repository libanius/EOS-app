...
| SIM-T12 | Simulador encena eventos no mapa | PENDING — **ideia do dono, 2026-08-13** | Furacão de mentira se aproximando, aviso de terremoto, e afins, desenhados no mapa durante o treino. Hoje o Simulador derruba FONTES (`isSourceDown`), mas não FABRICA evento — então a faixa de reavaliação de D-168 só aparece com alerta real na região. Não é só demonstração: é o único jeito de ensaiar a resposta antes do dia real. Precisa de decisão sobre onde o evento falso entra sem contaminar o snapshot verdadeiro |
...
| PILOT-T12 | Pilot diz "zero" quando na verdade não sabe | PENDING — **defeito de segurança, achado do dono 2026-08-13** | **Causa localizada**: `components/world-v2/usePilotFacts.ts:110` — quando `!casa.known \|\| casa.size <= 0`, os fatos viram `FATOS_VAZIOS`, com `waterDays/foodDays/powerDays/fuelDays/autonomyDays = 0`. Esses zeros viajam para o Pilot como se fossem MEDIDOS, e ele escreve "sua autonomia está em zero, o que significa que sua família não tem reservas". O comentário do próprio arquivo diz que a intenção era "dizer que não sabe" — mas zero não é ausência, é um fato, e o pior possível. `useWorldData` (painel) TEM fallback para o inventário pessoal; o Pilot não tem, e as duas telas divergem — o que `pilot-one-truth-test` existe para impedir. É o espelho de `unknown ≠ safe`: aqui virou `unknown = catástrofe`, que mente igual e custa mais confiança, porque o usuário SABE que tem água. Correção precisa distinguir "não sei" de "zero" no contrato enviado ao modelo. **Investigação aprofundada 2026-08-13**: a causa mais provável não é casa desconhecida, e sim **CORRIDA** — `PilotDock` chama `usePilotFacts(open)`, e os fatos só começam a carregar quando o orbe ABRE. Quem digita imediatamente envia contexto ainda vazio, e `FATOS_VAZIOS` tem todos os dias em ZERO. Confere com o print: a Preparação mostrava "3 membros" (casa conhecida) enquanto o Pilot dizia zero. **A correção certa cobre os dois casos**: fatos não carregados e casa desconhecida precisam viajar como `null`/"não sei", nunca como `0`. Considerar também: segurar o envio até os fatos chegarem, ou marcar a resposta como preliminar |
| PREP-T10 | Estados de aquisição (fase 1 de 4) | ✅ COMPLETE (migração **pendente de aplicação**) | 2026-08-13 — D-171. `docs/37` §28 diz que **cada estágio é entregável sozinho** e que o cutover é "explícito, decidido, uma tarefa" — então T10 vira quatro. Fase 1 é ADITIVA e não migra dado. Critérios binários: (1) coluna `status` em `checklists`, idempotente, derivada de `acquired` na criação; (2) `acquired` **continua sendo mantida** — nada de legado quebrado; (3) ciclo de vida como módulo puro com transições permitidas; (4) `met` continua vindo do usuário **até haver holdings reais**, e isso fica escrito — a interface não pode prometer o que o domínio ainda não sustenta; (5) `not_applicable` vira ação de verdade: o item some da conta de faltantes e **não volta** na próxima geração; (6) sem backfill, sem escrita dupla, sem cutover. **Entregue** 2026-08-13 — D-171: `lib/acquisition.ts` (16 testes), `status` na API, "não se aplica" na tela com desfazer, progresso ignorando descartado. 377 testes. **Pendência: aplicar `20260813180000_checklist_status.sql`** |
...
| PREP-T10d | Cutover | PENDING — **forma definida, execução não iniciada** | 2026-08-13 — D-173. Portão pronto e **verde**: `npm run test:cutover-gate` prova que os 3 perfis com dado batem entre legado e modelo novo. **A forma mudou por evidência**: o espelho invertido é impossível — `kit_type` guarda uma dimensão, e requisito com kit E procedência não cabe nele (hoje 0 casos, mas o primeiro item da Bug Out sugerido pelo Pilot torna a volta lossy para sempre). Logo o cutover **congela** `checklists` em vez de sincronizá-la, e precisa mover os **18 leitores de uma vez**. Deve ser feito em sessão própria, com verificação de navegador — é o primeiro passo irreversível da frente |
| PREP-T15 | Perfis órfãos: `profiles` não tem FK para `auth.users` | PENDING — **achado 2026-08-13** | 19 perfis para 9 contas. `profiles.id` é `uuid PRIMARY KEY DEFAULT auth.uid()` sem chave estrangeira, então apagar a conta deixa o perfil e tudo pendurado nele. 10 órfãos acumulados de meses de teste. Descoberto porque o backfill contou 16 requisitos para 15 itens. Precisa de decisão: adicionar FK com CASCADE (e o que fazer com os 10 existentes, um deles com nome de pessoa) |
