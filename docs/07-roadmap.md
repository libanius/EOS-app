...
| EXEC-T06 | Promoção de ponto do dia + basemap persistente | ✅ COMPLETE (migração pendente de aplicação) | 2026-08-19 — Encerramento oferece promover pontos do dia para `circle_places` com `precision: unknown`, recusar não perde o registro da execução, e `profiles.map_base_mode` vira preferência única de basemap para Mundo, `MapPointPicker` e `RouteDraw`. Aplicar `20260819144004_exec_t06_map_base_mode.sql`. |
