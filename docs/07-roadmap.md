...
| PREP-T11 | Checklist para de sobrescrever o estoque + água em galão FEMA | **PENDING — DESBLOQUEADA, EXECUTA PRIMEIRO** | 2026-08-12 — D-156 + D-158 + D-159. Aceitação binária: (1) marcar item de 4 gal numa casa com 20 gal mantém 20 gal, provado por teste; (2) água exibida em galão em Preparação, Pilot, veredito e debrief; (3) constante única = 1 gal FEMA, as 3 cópias do literal `3` consolidadas; (4) `household.test.ts:73` atualizado no mesmo commit; (5) nota única ao usuário explicando a régua nova; (6) limiares convertidos **1:1**, sem mudar rigor (isso é PREP-T13); (7) **armazenamento continua em litros** — `water_liters` só é renomeado na migração de PREP-T04; (8) sem migração, sem mudança de rota |
...
| PREP-T13 | Revisar limiares da nota de água | PENDING | 2026-08-12 — D-159 levantou: `threshold={4}`/`criticalThreshold={2}` L por pessoa são **absolutos, não por dia** — "adequado" significa 1 dia de água, contra o mínimo FEMA de 3 dias. Decisão de severidade própria, para não embutir duas mudanças na mesma entrega |
| PREP-T04 | Holdings + Location padrão (fundação de inventário) | PENDING | Depois de PREP-T11. `holdings` + `locations`, "Casa" criada por padrão, adaptador lendo `resource_inventory` + itens `acquired`. Autonomia = consumíveis sob CASA (D-156). Critérios binários entram na tarefa antes de código |
