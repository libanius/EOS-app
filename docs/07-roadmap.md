...
| MAP-T06 | O cone da tempestade não cabe no enquadramento | PENDING — **defeito anterior, confirmado** | `test:weather` reprova `enquadramento da tempestade` com `zoom 4.4`, cone `22.0°×6.8°` e vista `14.0°×28.8°`. Verificado guardando o trabalho de D-199 e rodando o código anterior: **números idênticos**, logo é anterior. O `fitBounds` recebe `stormBounds(...)` e o teste cobra a caixa do CONE — falta descobrir se são a mesma caixa |
