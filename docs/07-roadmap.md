...
| EXEC-T03 | `family_plan_executions` + disparo por gesto | IMPLEMENTED — MIGRATION PENDING | 2026-08-19 — Migration local criada, API cria execução + evento `started` + aviso no mesmo ato, `MemberSheet` virou seleção/hold de 1,5 s, banner global mostra undo por 30 s sem modal. Aplicar `20260819124613_exec_t03_family_plan_executions.sql` antes de validar em produção. |
