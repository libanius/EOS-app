...
| WV2-T32 | Camadas NHC operacionais no Mundo | IN PROGRESS | D-223/D-224. Separar centro atual, trajetória, pontos, cone, trajetória passada e watches/warnings; adicionar legenda operacional NHC; preservar compatibilidade com `cyclone`; não inferir trajetória proprietária ou evacuação. |
...
| EXEC-T04 | Playbook por papel offline-first | ✅ COMPLETE | 2026-08-19 — Playbook global lê documento por `(circleId, planId)` do cache antes da rede, escolhe protocolo em `family_plan_executions.protocol_index`, lista passos por usuário, calcula rumo/distância/minutos sem tile e separa carta do dependente/avisos da numeração. Migration `20260819131113_exec_t04_dependent_briefs.sql` aplicada e verificada por REST. D-214: brilho máximo real fica para Native futuro; PWA só aplica legibilidade e números ampliados. |
...
| ALERT-T08 | Agendador no pg_cron do Supabase; aposentar o `schedule:` do GitHub | ✅ COMPLETE (código) / PENDENTE (dono) | 2026-08-28 — D-222. O GitHub entregava **12,7%** do agendado: 67 execuções onde caberiam 526 em 87,7 h, mediana de 43 min, maior buraco de **11,6 h**, zero falhas — elas não falham, não acontecem. `supabase/pg_cron_hazard_scan.sql` agenda dentro do banco com o `CRON_SECRET` no **Vault** (o corpo do job é legível em catálogo e backup). O GitHub **fica ligado** até o novo provar-se: critério para remover o `schedule:` é 24 h de respostas a cada 10 min em `net._http_response` |
| ALERT-T09 | Índice de dedup volta a arbitrar `ON CONFLICT` + nenhuma escrita muda | ✅ COMPLETE (código) / PENDENTE (dono aplicar migration) | 2026-08-28 — D-222. `uq_ndl_user_dedup` era **parcial**, e índice parcial não arbitra `ON CONFLICT` sem repetir o predicado — que o `on_conflict` do PostgREST não emite. Toda escrita do log de entrega voltava `42P10` e era descartada sem checagem: dedup e cooldown estavam **inertes desde o primeiro minuto** em produção, com o resumo relatando `pushed: 1` contra tabela vazia. Migration `20260828000000_ndl_dedup_arbiter.sql` + helper `guard` nas 5 escritas de `scan.ts`. Regressão: `npm run test:hazard-dedup` (5 asserções + controle negativo) |
| I18N-T01 | Coerência de idioma e unidade — leva 1: Alertas + Live Intelligence Network | ✅ COMPLETE | 2026-08-28 — D-225. `lib/display-units.ts` (23 testes): inglês→imperial, português→métrico, conversão só na borda. ~130 chaves novas no `lib/i18n.tsx` cobrindo AlertsPage, LIN, 6 classes de perigo, 6 categorias, 30 atividades e 5 kits — as atividades só existiam em inglês e os kits só em português, na mesma tela. `t()` ganhou interpolação `{marcador}`. Resumo do ciclone e estado da rede passam a ser renderizados de `metrics`/`headlineKey` em vez do texto pronto do servidor. Corrigidas duas falhas silenciosas (`saveToKit`, `handleCustomActivity`). `npm run test:alerts-i18n` 9/9 em navegador real, com controle negativo. **Levas 2 e 3 PENDENTES**: `MemberSheet`, `CircleSheets`, `ChecklistDialogs`; depois `RouteDraw`, `MapPointPicker`, `PlanChart`, `DetentSheet` — todas com texto cravado em português num app cujo padrão é inglês |
| I18N-T02 | Unificar os três padrões de tradução num só | PENDING | D-225 follow-up. **Não é defeito de idioma** — as telas são bilíngues e corretas; é dívida de manutenção. Três padrões convivem: `t()` do `lib/i18n.tsx` (44 arquivos), ternário inline `pt ? 'x' : 'y'` (`MemberSheet`, `CircleSheets`, `ChecklistDialogs`) e objeto `COPY = { pt, en }` (`RouteDraw`, `MapPointPicker`). O custo é ter três lugares para procurar uma frase; o usuário não vê diferença. ~1.100 linhas sem mudança visível, então **só vale junto com outro trabalho nesses arquivos**, e com um teste que compare a tela nos dois idiomas antes e depois. Registrado depois de um diagnóstico ERRADO meu, que acusou 'português cravado' contando letras acentuadas — uma tela bilíngue tem tantos acentos quanto uma monolíngue |
