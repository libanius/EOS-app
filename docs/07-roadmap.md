...
| NAV-T05 | Círculos e Ficha entram em Família | PENDING | Fase 2. Libera dois slots da barra. `/circles` e `/ficha` redirecionam; `/ficha/[id]` (QR público) **não muda de endereço** — está impresso e compartilhado |
| NAV-T06 | Barra encolhe para 5; `/mais` absorve o ☰ | PENDING | Fase 3. `[ FAMÍLIA ] [ PREPARAÇÃO ] (( MUNDO )) [ COMMS ] [ MAIS ]`. Só depois das absorções, que são o que libera os slots |
| NAV-T07 | `/weather` desce para dentro do MUNDO | PENDING | Fase 4. Fim da duplicação: alertas e condições em duas telas e duas linguagens visuais |
| NAV-T08 | Cenário vira MODO, não destino | PENDING | Fase 5. `SimulationProvider` já é global e já reconfigura o app; ocupar slot de destino é erro de categoria |
