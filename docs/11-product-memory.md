...
- Client subscribe must not await `navigator.serviceWorker.ready` without a timeout; if SW registration stalls, the Settings button can remain in loading. Convert `NEXT_PUBLIC_VAPID_PUBLIC_KEY` from URL-safe base64 to `Uint8Array` before `pushManager.subscribe`, request `Notification` permission explicitly, and surface API errors in the UI.
